...
init_per_suite(Config) ->
    ...
    MnesiaDir = "/tmp/shurbej_test_mnesia_" ++ integer_to_list(erlang:unique_integer([positive])),
    application:set_env(mnesia, dir, MnesiaDir),
    application:set_env(shurbej, http_port, 18080),
...
file_content_addressed_dedup(Config) ->
    ...
    [{shurbej_blob, Sha256, _, 2}] = mnesia:dirty_read(shurbej_blob, Sha256),
    ok.
...
session_expired(_Config) ->
    ...
    shurbej_session:insert_raw(Token, #{
        status => pending,
        created => erlang:system_time(millisecond) - 700000,  %% 11+ minutes ago
        login_url => <<"http://localhost/login">>
    }),
    {error, expired} = shurbej_session:get(Token),
    ok.
...
session_cleaner_runs(_Config) ->
    ...
    shurbej_session_cleaner ! cleanup,
    timer:sleep(200),
    %% Session should be gone
    ?assertEqual([], ets:lookup(shurbej_login_sessions, Token)),
    %% Also test upload cleanup via the gen_server API
    ok = shurbej_files:cleanup_expired_uploads(),
    ok.
...
write_token_store_and_cleanup(_Config) ->
    %% Store a token
    ok = shurbej_write_token:store(<<"test_wt_1">>, {some_result, 1}),
    %% Should be duplicate now
    {duplicate, {some_result, 1}} = shurbej_write_token:check(<<"test_wt_1">>),
    %% Trigger cleanup
    shurbej_write_token ! cleanup,
    timer:sleep(100),
    %% Token should still exist (not expired yet)
    {duplicate, _} = shurbej_write_token:check(<<"test_wt_1">>),
    ok.
...
duplicate_user_create(_Config) ->
    ?assertEqual({error, already_exists},
                 shurbej_admin:create_user(<<"testuser">>, <<"other">>, 2)),
    ok.
