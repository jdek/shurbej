...
init(LibraryId) ->
    case shurbey_db:get_library(LibraryId) of
        {ok, #shurbey_library{version = Version}} ->
...
do_write(LibId, Current, WriteFun, State) ->
    ...
    case mnesia:transaction(fun() ->
        ...
        {atomic, ok} ->
            ...
            try
                Members = pg:get_members(shurbey_stream, Topic),
...
call(LibraryId, Msg) ->
    case global:whereis_name({?MODULE, LibraryId}) of
        undefined ->
            case shurbey_version_sup:start_child(LibraryId) of
                {ok, Pid} ->
