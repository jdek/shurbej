...
handle(<<"GET">>, Req0, State) ->
    ...
    case shurbey_http_common:get_if_modified(Req0) of
        V when is_integer(V), V >= LibVersion ->
            ...
            {ok, Req, State};
        _ ->
            Searches0 = shurbey_db:list_searches(LibId, Since),
            Searches = shurbey_http_common:filter_by_keys(Searches0,
                shurbey_http_common:get_search_keys(Req0)),
            Format = shurbey_http_common:get_format(Req0),
            case Format of
                <<"versions">> ->
                    Pairs = shurbey_db:list_search_versions(LibId, Since),
                    Req = shurbey_http_common:json_response(200, maps:from_list(Pairs), LibVersion, Req0),
                    {ok, Req, State};
                <<"keys">> ->
                    Keys = [K || #shurbey_search{id = {_, K}} <- Searches],
                    Req = shurbey_http_common:json_response(200, Keys, LibVersion, Req0),
                    {ok, Req, State};
                _ ->
                    Req = shurbey_http_common:list_response(Req0, Searches, LibVersion,
                        fun(S) -> shurbey_http_common:envelope_search(LibId, S) end),
                    {ok, Req, State}
            end
    end;
...
handle(Method, Req0, #{scope := single} = State) when Method =:= <<"PUT">>; Method =:= <<"PATCH">> ->
    ...
    case [S || #shurbey_search{id = {_, K}} = S <- Searches, K =:= SearchKey] of
        ...
        [#shurbey_search{data = Existing}] ->
            ...
            case shurbey_validate:search(Search) of
                {error, Reason} ->
                ...
                ok ->
                    case shurbey_version:write(LibId, ExpectedVersion, fun(NewVersion) ->
                        ...
                        shurbey_db:write_search(#shurbey_search{
...
handle(<<"POST">>, Req0, State) ->
    ...
    {Valid, Failed} = shurbey_http_items:validate_each(KeyedSearches, fun shurbey_validate:search/1),
    case Valid of
            ...
            case shurbey_version:write(LibId, ExpectedVersion, fun(NewVersion) ->
                ...
                {ok, NewVersion} ->
                    Successful = maps:from_list(
                        [{integer_to_binary(Idx), shurbey_http_items:envelope_for_write(LibId, S, NewVersion)}
                         || {Idx, S} <- Valid]),
                    Result = #{
...
handle(<<"DELETE">>, Req0, State) ->
    ...
    case shurbey_version:write(LibId, ExpectedVersion, fun(NewVersion) ->
        lists:foreach(fun(K) ->
            shurbey_db:mark_search_deleted(LibId, K, NewVersion),
            shurbey_db:record_deletion(LibId, <<"search">>, K, NewVersion)
        end, Keys),
        ok
    end) of
        {ok, NewVersion} ->
