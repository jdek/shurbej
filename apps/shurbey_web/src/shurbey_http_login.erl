                        ...
                        ok ->
                            case shurbey_db:authenticate_user(Username, Password) of
                                {ok, UserId} ->
                                    ...
                                    shurbey_db:create_key(ApiKey, UserId, #{library => true, write => true, files => true, notes => true}),
                                    UserInfo = #{user_id => UserId, username => Username, display_name => Username},
                                    ok = shurbey_session:complete(Token, ApiKey, UserInfo),
