        ...
        Key ->
            case shurbey_auth:key_info(Key) of
                {ok, #{user_id := UserId, permissions := Perms}} ->
                    put(shurbey_perms, normalize_perms(Perms)),
                    {ok, UserId, Req};
                {error, _} -> {error, Req}
            end
    end.
...
authorize(Req) ->
    case authenticate(Req) of
        {ok, UserId, Req2} ->
            case shurbey_rate_limit:check(UserId) of
                {error, rate_limited} ->
...
sort_key_fn(<<"dateModified">>) ->
    fun(#shurbey_item{data = D}) -> maps:get(<<"dateModified">>, D, <<>>);
       (#shurbey_collection{data = D}) -> maps:get(<<"dateModified">>, D, <<>>);
       (#shurbey_search{data = D}) -> maps:get(<<"dateModified">>, D, <<>>)
    end;
sort_key_fn(<<"dateAdded">>) ->
    fun(#shurbey_item{data = D}) -> maps:get(<<"dateAdded">>, D, <<>>);
       (#shurbey_collection{data = D}) -> maps:get(<<"dateAdded">>, D, <<>>);
       (#shurbey_search{data = D}) -> maps:get(<<"dateAdded">>, D, <<>>)
    end;
sort_key_fn(<<"title">>) ->
    fun(#shurbey_item{data = D}) -> maps:get(<<"title">>, D, <<>>);
       (#shurbey_collection{data = D}) -> maps:get(<<"name">>, D, <<>>);
       (#shurbey_search{data = D}) -> maps:get(<<"name">>, D, <<>>)
    end;
sort_key_fn(<<"creator">>) ->
    fun(#shurbey_item{data = D}) ->
...
sort_key_fn(<<"itemType">>) ->
    fun(#shurbey_item{data = D}) -> maps:get(<<"itemType">>, D, <<>>);
       (_) -> <<>>
    end;
sort_key_fn(_) ->
    fun(#shurbey_item{version = V}) -> V;
       (#shurbey_collection{version = V}) -> V;
       (#shurbey_search{version = V}) -> V
    end.
...
record_key(#shurbey_collection{id = {_, K}}) -> K;
record_key(#shurbey_search{id = {_, K}}) -> K;
record_key(#shurbey_item{id = {_, K}}) -> K.

filter_by_tag(Items, none) -> Items;
filter_by_tag(Items, Tag) ->
    [I || #shurbey_item{data = D} = I <- Items,
...
base_url() ->
    to_binary(application:get_env(shurbey, base_url, <<"http://localhost:8080">>)).

envelope_item(LibId, Item) ->
    envelope_item(LibId, Item, #{}).

envelope_item(LibId, #shurbey_item{id = {_, Key}, version = Version, data = Data}, ChildrenCounts) ->
    Base = base_url(),
...
envelope_collection(LibId, #shurbey_collection{id = {_, Key}, version = Version, data = Data}) ->
    Base = base_url(),
    LibBin = integer_to_binary(LibId),
...
envelope_search(LibId, #shurbey_search{id = {_, Key}, version = Version, data = Data}) ->
    Base = base_url(),
    LibBin = integer_to_binary(LibId),
    #{
        <<"key">> => Key,
